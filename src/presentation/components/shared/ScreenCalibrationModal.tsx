...
import { FiX, FiMaximize, FiInfo } from 'react-icons/fi';
            ...
            <div className="bg-white w-full max-w-2xl rounded-3xl shadow-xl overflow-hidden flex flex-col relative animate-fade-in-up">
                ...
                <div className="p-6 flex flex-col gap-8">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3 text-blue-800 text-sm">
                        ...
                        <div>
                            <p><strong>Kenapa kalibrasi diperlukan?</strong> Ukuran piksel pada monitor (PPI) berbeda-beda. Agar pembacaan grid EKG 1mm = 0.04s akurat secara fisik, sesuaikan lebar kotak di bawah ini agar sama persis dengan penggaris di dunia nyata.</p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <div className="flex bg-slate-100 rounded-lg p-1">
                            <button 
                                onClick={() => setTargetLengthCm(5)}
                                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${targetLengthCm === 5 ? 'bg-white shadow-sm text-charcoal' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                5 cm
                            </button>
                            <button 
                                onClick={() => setTargetLengthCm(8.56)}
                                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${targetLengthCm === 8.56 ? 'bg-white shadow-sm text-charcoal' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Kartu ATM / KTP (8.56 cm)
                            </button>
                            <button 
                                onClick={() => setTargetLengthCm(10)}
                                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${targetLengthCm === 10 ? 'bg-white shadow-sm text-charcoal' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                10 cm
                            </button>
                        </div>
                    ...
                    <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-slate-200 min-h-[160px] overflow-x-auto custom-scrollbar">
                        <div className="relative flex flex-col items-center">
                            ...
                            <div 
                                className="bg-medical-teal rounded-sm shadow-sm transition-all duration-75 relative flex items-center justify-center"
                                style={{ width: `${boxWidthPx}px`, height: '54px' }}
                            >
                                {/* Ruler ticks */}
                                ...
                                <span className="text-white font-mono font-bold text-sm select-none">
                                    {targetLengthCm} cm
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-4 items-center">
                            <button 
                                onClick={() => setPixelsPerMm(p => Math.max(1, p - 0.1))}
                            ...
                            />
                            <button 
                                onClick={() => setPixelsPerMm(p => Math.min(8, p + 0.1))}
                                className="w-8 h-8 flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-full font-bold"
                            >
                                +
                            </button>
                ...
                <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button 
                            onClick={handleSave}
                            className="px-5 py-2 rounded-xl text-sm font-bold bg-medical-teal hover:bg-teal-600 text-white shadow-sm transition-colors"
                        >
                            Simpan Kalibrasi
                        </button>
                    </div>
