...
import { CALIBRATION_POINT_COUNT } from "../../../core/algorithms/ecgFrameProcessor";
...
export const EcgViewer: React.FC<EcgViewerProps> = ({ segment, speed = 25, timeOffset = 0, classResult }) => {
  ...
  const raw = segment?.payload?.raw || {};
  const samples = raw.ch1?.length ? raw.ch1 : segment?.payload?.ecg?.samples || [];
  const ch2 = raw.ch2 || [];
  const ch3 = raw.ch3 || [];
  ...
  const animate = (time: number) => {
    ...
    const rate = 250 * playbackSpeedRef.current; // 250 Hz sampling rate * speed multiplier
    ...
    <div className="relative flex flex-col flex-1 h-full w-full">
      ...
      <div className="flex-1 min-h-0 relative flex flex-col">
        ...
        <div className="absolute top-3 right-3 z-30 pointer-events-none select-none flex flex-wrap gap-1.5 max-w-[70%] justify-end">
          {isFilterOn ? (
            <>
              <span className="bg-blue-50/90 backdrop-blur-xs border border-blue-100/50 text-clinical-blue text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-xs">Filter: ON</span>
              {activeFilters.baseline && <span className="bg-emerald-50/90 backdrop-blur-xs border border-emerald-100/50 text-emerald-600 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-xs">Baseline</span>}
              {activeFilters.denoise && <span className="bg-emerald-50/90 backdrop-blur-xs border border-emerald-100/50 text-emerald-600 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-xs">Denoise</span>}
              {activeFilters.bandpass && <span className="bg-emerald-50/90 backdrop-blur-xs border border-emerald-100/50 text-emerald-600 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-xs">Bandpass</span>}
              {activeFilters.normalization && <span className="bg-purple-50/90 backdrop-blur-xs border border-purple-100/50 text-purple-600 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-xs">Normalized</span>}
            </>
          ) : (
            <span className="bg-amber-50/90 backdrop-blur-xs border border-amber-100/50 text-amber-600 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-xs">Raw Data (No Filters)</span>
          )}
        </div>
      </div>
        ...
        <div className="flex-shrink-0 bg-white border-t border-clinical-charcoal/5 px-6 py-3.5 flex flex-col xl:flex-row items-center justify-between gap-4 z-20 select-none">
          ...
          <div className="flex items-center gap-3 w-full xl:w-auto justify-between xl:justify-start">
            ...
            <div className="ml-2 flex flex-col justify-center">
              <span className="text-[9px] font-bold text-clinical-charcoal/40 uppercase tracking-wider leading-none">{t("history.recordingTime")}</span>
          ...
          <div className="flex-1 flex items-center gap-3 w-full px-2">
            ...
            />
            <span className="text-xs font-bold text-clinical-charcoal/50 w-10 text-right font-mono-data">{progressPercentage}%</span>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap justify-end w-full xl:w-auto">
            ...
            <div className="flex items-center gap-1 bg-clinical-surface p-1 rounded-full border border-clinical-charcoal/5 relative" ref={filterMenuRef}>
              ...
              >
                <span className="material-symbols-outlined text-[14px]">{isFilterOn ? "filter_alt" : "filter_alt_off"}</span>
              ...
              {isFilterOn && (
                ...
                >
                  <span className="material-symbols-outlined text-[16px] font-bold">settings</span>
                </button>
              )}
              ...
              {isFilterOn && showFilterMenu && (
                <div className="absolute bottom-full right-0 mb-2 bg-white border border-clinical-charcoal/10 rounded-[1.5rem] p-4 shadow-xl z-[200] min-w-[220px] flex flex-col gap-2.5 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  <h4 className="text-[10px] font-bold text-clinical-charcoal/40 uppercase tracking-widest px-1">Pilih Filter Aktif</h4>

                  <label className="flex items-center gap-2 px-1 hover:bg-clinical-surface/50 py-1.5 rounded cursor-pointer select-none">
                    <input type="checkbox" checked={activeFilters.baseline} onChange={(e) => toggleSubFilter("baseline", e.target.checked)} className="w-3.5 h-3.5 rounded border-gray-300 text-clinical-blue focus:ring-clinical-blue" />
                  ...
                  <label className="flex items-center gap-2 px-1 hover:bg-clinical-surface/50 py-1.5 rounded cursor-pointer select-none">
                    <input type="checkbox" checked={activeFilters.denoise} onChange={(e) => toggleSubFilter("denoise", e.target.checked)} className="w-3.5 h-3.5 rounded border-gray-300 text-clinical-blue focus:ring-clinical-blue" />
                    <div className="flex flex-col">
                  ...
                  <label className="flex items-center gap-2 px-1 hover:bg-clinical-surface/50 py-1.5 rounded cursor-pointer select-none">
                    <input type="checkbox" checked={activeFilters.bandpass} onChange={(e) => toggleSubFilter("bandpass", e.target.checked)} className="w-3.5 h-3.5 rounded border-gray-300 text-clinical-blue focus:ring-clinical-blue" />
                    <div className="flex flex-col">
